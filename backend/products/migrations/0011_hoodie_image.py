...
def seed_image(apps, schema_editor):
    ...
    product = Product.objects.using(db).filter(name="Hoodie", garment_type="hoodie").first()
    ...
    name = "products/hoodie-catalog.webp"
    if not storage.exists(name):
        data = (Path(__file__).resolve().parent.parent / "assets" / "hoodie.webp").read_bytes()
...
class Migration(migrations.Migration):
    dependencies = [("products", "0010_commercial_catalog")]
