...
class CartApiTests(TestCase):
    ...
    def test_anonymous_cannot_add_to_cart(self):
        response = self.client.post('/api/cart/items/', {'product': self.product_x.id, 'variant': self.variant_x.id}, format='json')
    ...
    def test_customer_can_change_quantity_and_delete_item(self):
        ...
        response = self.client.patch(f'/api/cart/items/{item.id}/', {'quantity': 3}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['quantity'], 3)
        response = self.client.patch(f'/api/cart/items/{item.id}/', {'quantity': 1}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['quantity'], 1)
        self.assertEqual(self.client.delete(f'/api/cart/items/{item.id}/').status_code, status.HTTP_204_NO_CONTENT)

    def test_customer_can_empty_cart_and_count_is_total_units(self):
        self.client.force_authenticate(user=self.user_a)
        self.client.post('/api/cart/items/', {'product': self.product_x.id, 'variant': self.variant_x.id, 'quantity': 2}, format='json')
        self.client.post('/api/cart/items/', {'product': self.product_y.id, 'variant': self.variant_y.id, 'quantity': 3}, format='json')
        response = self.client.get('/api/cart/')
        self.assertEqual(response.data['item_count'], 5)
        self.assertEqual(self.client.delete('/api/cart/').status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(self.client.get('/api/cart/').data['items'], [])

    def test_user_cannot_modify_another_users_item(self):
        self.client.force_authenticate(user=self.user_a)
        self.client.post('/api/cart/items/', {'product': self.product_x.id, 'variant': self.variant_x.id}, format='json')
        item = CartItem.objects.get(cart__user=self.user_a)
        self.client.force_authenticate(user=self.user_b)
        self.assertEqual(self.client.patch(f'/api/cart/items/{item.id}/', {'quantity': 2}, format='json').status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(self.client.delete(f'/api/cart/items/{item.id}/').status_code, status.HTTP_404_NOT_FOUND)

    def test_variant_from_another_product_is_rejected(self):
        self.client.force_authenticate(user=self.user_a)
        response = self.client.post('/api/cart/items/', {'product': self.product_x.id, 'variant': self.variant_y.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('variant', response.data)

    def test_product_with_variants_requires_variant(self):
        self.client.force_authenticate(user=self.user_a)
        response = self.client.post('/api/cart/items/', {'product': self.product_x.id}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data['variant'][0], 'Selecciona una variante antes de agregar el producto al carrito.')

    def customization(self, content='GYM CULTURE'):
        element = {
            'id': 'text-1', 'type': 'text', 'content': content,
            'x': 50, 'y': 50, 'width': 55, 'height': 18,
            'rotation': 15, 'font': 'Outfit', 'bold': True,
        }
        return {
            'version': 1,
            'garment': 'tshirt',
    ...
    def test_customized_item_saves_snapshot_and_previews(self):
        self.client.force_authenticate(user=self.user_a)
        response = self.client.post('/api/cart/items/', self.customized_payload(), format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        item = CartItem.objects.get(cart__user=self.user_a)
        self.assertEqual(item.customization_data['garments']['tshirt']['front']['elements'][0]['content'], 'GYM CULTURE')
    ...
    def test_customization_image_is_persisted_as_media_reference(self):
        ...
        payload['customization_data']['garments']['tshirt']['front']['elements'].append({
            'id': 'image-1', 'type': 'image', 'content': self.preview_data,
            'x': 40, 'y': 40, 'width': 30, 'height': 30, 'rotation': 0,
        })
        response = self.client.post('/api/cart/items/', payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        item = CartItem.objects.get(cart__user=self.user_a)
        image_content = item.customization_data['garments']['tshirt']['front']['elements'][1]['content']
    ...
    def test_different_customizations_are_separate_and_same_is_reused(self):
        self.client.force_authenticate(user=self.user_a)
        first = self.client.post('/api/cart/items/', self.customized_payload('GYM CULTURE'), format='json')
        second = self.client.post('/api/cart/items/', self.customized_payload('NO PAIN NO GAIN'), format='json')
        same = self.client.post('/api/cart/items/', self.customized_payload('GYM CULTURE'), format='json')
    ...
    def test_customer_can_update_customization_without_duplicate(self):
        self.client.force_authenticate(user=self.user_a)
        self.client.post('/api/cart/items/', self.customized_payload('GYM CULTURE'), format='json')
        item = CartItem.objects.get(cart__user=self.user_a)
        response = self.client.patch(f'/api/cart/items/{item.id}/', self.customized_payload('EDITED'), format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(CartItem.objects.filter(cart__user=self.user_a).count(), 1)
        self.assertEqual(CartItem.objects.get(pk=item.id).customization_data['garments']['tshirt']['front']['elements'][0]['content'], 'EDITED')
    ...
    def test_customer_cannot_read_or_update_another_users_customization(self):
        ...
        item = CartItem.objects.get(cart__user=self.user_a)
        self.client.force_authenticate(user=self.user_b)
        self.assertEqual(self.client.get(f'/api/cart/items/{item.id}/').status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(self.client.patch(f'/api/cart/items/{item.id}/', self.customized_payload('HACKED'), format='json').status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_invalid_customization_and_image_are_rejected(self):
        ...
        self.assertEqual(self.client.post('/api/cart/items/', invalid, format='json').status_code, status.HTTP_400_BAD_REQUEST)
        invalid_image = self.customized_payload()
        invalid_image['preview_front'] = 'data:image/svg+xml;base64,PHN2Zz48L3N2Zz4='
        self.assertEqual(self.client.post('/api/cart/items/', invalid_image, format='json').status_code, status.HTTP_400_BAD_REQUEST)
