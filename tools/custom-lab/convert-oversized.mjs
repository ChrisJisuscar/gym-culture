...
const source = 'frontend/static/models/Low_T-shirt.fbx';
const destination = 'frontend/static/models/oversized.glb';
...
const plain = new THREE.MeshStandardMaterial({
  name: 'oversized_plain_fabric', color: '#ffffff', metalness: 0, roughness: 0.88, side: THREE.DoubleSide,
});
model.traverse((object) => {
  ...
  report.meshes.push({
    name: object.name, vertices: geometry.attributes.position.count,
...
const validation = await validator.validateBytes(new Uint8Array(glb), { uri: 'oversized.glb' });
...
await fs.writeFile('tools/custom-lab/oversized-inspection.json', JSON.stringify(report, null, 2) + '\n');
