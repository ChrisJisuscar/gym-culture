...
exports.remove = fixture => run("import json,sys; from users.models import User; data=json.load(sys.stdin); assert data['admin'].startswith('qa-session-') and data['customer'].startswith('qa-customer-'); User.objects.filter(username__in=[data['admin'],data['customer']],email__endswith='@example.invalid').delete(); print('clean')", JSON.stringify({ admin: fixture.admin, customer: fixture.customer }));
